...
function NavLink({ children, icon, ...others }) {
  ...
  return (
    <Link
      {...others}
      className={`${isActive && "pointer-events-none"} overflow-hidden relative font-medium flex text-2xl gap-4 p-4 items-center justify-center lg:justify-normal border-y border-transparent  hover:border-grays-200 hover:bg-grays-100 transition-all`}
    >
      <AnimatePresence>
        {isActive && (
